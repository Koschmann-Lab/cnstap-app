...
<br><br>
    <div id="pathwayGraph" v-if="switch2">
      ...
      <!-- Pathway Big Graph - Start -->
        ...
        <!-- Pathway Big Graph - End -->


         <!-- LineCharts Start -->
        <div v-for="testsetObj in tempGraphDataset"
...
//      testLineGraph,
    ...
    { pathways: "ALK", drugagents: "Ponatinib", vitro: 4, vivo: 0, safety: 3, cns: 10, bbb: 10, fda: 0, subt:27, icln: 4, itier: 0, itrl: 0,  cln: 4, tier: 0, trl: 0, total: 31, editable: 0, },
    ...
    testset: [
      ...
      {pathway: "BRAF", drugagents:[
        {agent:"Enrectinib",values:{baseline:27, ptsepcific:52}},
      ]},
    ],

    tempGraphDataset: [{ pathway: "", drugagents:[]},],
         ...
         filterGraphData() {
              if(this.pathwayselection.length==0){
                ...
                this.switch2=false;
                      ...
                      this.tempGraphDataset.splice(0);
                      for (var p = 0; p < this.pathwayselection.length; p++){
                            var tDrugagents = [{ agent: "", values:{baseline:0, ptsepcific:0}},];
                            ...
                            //    this.tempDrugagents.push({agent:this.drugs[d].drugagents, values:{baseline:this.drugs[d].subt, ptsepcific:this.drugs[d].total}});
                                  tDrugagents.push({agent:this.drugs[d].drugagents, values:{baseline:this.drugs[d].subt, ptsepcific:this.drugs[d].total}});
                            //    alert('drugagent:'+this.drugs[d].drugagents+'baseline:'+this.drugs[d].subt+'ptsepsific:'+this.drugs[d].total);

                              }
                            }
                          //    alert(JSON.stringify(this.tempDrugagents));
                          //    this.tempGraphDataset.push({pathway:this.pathways[this.pathwayselection[p]].pathway.trim(), drugagents:this.tempDrugagents});
                              this.tempGraphDataset.push({pathway:this.pathways[this.pathwayselection[p]].pathway.trim(), drugagents:tDrugagents});
                          //    alert(JSON.stringify(this.tempGraphDataset));
                        }
              }
    //          tempDrugagents: [{ agent: "", values:{baseline:0, ptsepcific:0}},],
    //          tempGraphDataset: [{ pathway: "", drugagents:[]},],
         ...
         loadGraphData(){
            ...
            //           alert("LineGraphDataset.length:" + this.LineGraphDataset.length);
                      this.chunk();
//                      this.LGDataset.push({pathway:this.oPathway,drugagents:this.LGdrugagents});
...
//                  alert('RowCount:' + this.RowCount + '/Tablerows:' + this.drugs.length);

//                  alert("oPathway:" + this.oPathway + "/pPathway:" + pPathway +"/pDrug:" + pDrug);
//                  alert(pPathway + " " + pDrug);
//                  if (((this.oPathway != pPathway) && (this.oPathway != "")) || (this.RowCount === this.drugs.length)    ) {
                    if ((this.oPathway != pPathway) && (this.oPathway != "")) {

//                        alert("LGdrugagents length:" + this.LGdrugagents.length);
//                        this.LGDataset.push({pathway:this.oPathway,drugagents:this.LGdrugagents});
...
//var output = '';
//for (var property in this.LGdrugagents) {
...
//alert(output);
                        ...
                        this.LGdrugagents.splice(0);
                  }

                  this.LGdrugagents.push({agent:pDrug,values:{baseline:pSubt, ptsepcific:pTotal}});
  //              alert(pPathway + ' ' + this.LGdrugagents[this.LGdrugagents.length-1].agent + ' ' + this.LGdrugagents[this.LGdrugagents.length-1].values.baseline + ' ' + this.LGdrugagents[this.LGdrugagents.length-1].values.ptsepcific);


                  this.oPathway = pPathway;
